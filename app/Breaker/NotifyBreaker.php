...
/**
 * the breaker of notify
 *
 * @Breaker("notify")
 */
class NotifyBreaker extends CircuitBreaker
{
    /**
     * The number of successive failures
     * If the arrival, the state switch to open
     *
     * @Value(name="${config.breaker.notify.failCount}", env="${NOTIFY_BREAKER_FAIL_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.notify.successCount}", env="${NOTIFY_BREAKER_SUCCESS_COUNT}")
     * @var int
     */
     ...
     *
     * @Value(name="${config.breaker.notify.delayTime}", env="${NOTIFY_BREAKER_DELAY_TIME}")
     * @var int
     */
